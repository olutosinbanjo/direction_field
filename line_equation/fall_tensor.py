...
def solutions(x_ax, y_ax):
    ...
    for i in x_ax:
        for j in y_ax:    
            ...
            X = tf.constant(np.linspace(i-0.2, i+0.2, 2))
...
def main():
    ...
    nx, ny = 1, 1
    x_axis = np.arange(1, 11, nx) #tf.experimental.numpy.arange(1, 11, nx)
    y_axis = np.arange(20, 60, ny) #tf.experimental.numpy.arange(20, 60, ny)
